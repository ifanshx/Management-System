...
<head>
    ...
    <title><?= esc($title ?? 'BOM Produk') ?> | <?= esc($company['app_name'] ?? 'Noric ERP') ?></title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
            ...
            --accent: #2563eb; --warning: #b45309; --danger: #b91c1c; --success: #166534;
        }
        ...
        .btn-print { background: linear-gradient(135deg, #2563eb, #1d4ed8); color: #fff; border: none; padding: 11px 22px; font-size: 13px; font-weight: 800; border-radius: 10px; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 8px 20px rgba(37, 99, 235, 0.22); transition: all .2s ease; }
        ...
        .qty-val { font-family: 'Space Mono', monospace; font-size: 11px; font-weight: 700; white-space: nowrap; text-align: center; }
        .qty-target { font-family: 'Space Mono', monospace; font-size: 10px; font-weight: 900; background: #eff6ff; color: #1e3a8a; display: inline-block; padding: 4px 8px; border: 1px solid #93c5fd; border-radius: 8px; min-width: 60px; text-align: center; text-transform: uppercase; }
        
        .final-badge { display: inline-block; font-size: 8px; font-weight: 900; color: #166534; background: #ecfdf5; border: 1px solid #a7f3d0; border-radius: 999px; padding: 2px 7px; margin-left: 6px; }
        ...
        .sig-box { width: 32%; border: 1px solid var(--line); border-radius: 10px; padding: 10px 8px 8px; background: rgba(255, 255, 255, 0.95); }
        ...
        @media print {
            ...
            .meta-grid, .notes-section, .signature-section, .header, .top-strip, .section-header, .table-wrap, .footer-note { page-break-inside: avoid !important; }
        }
    </style>
</head>
<body>
    ...
    <?php
        // =========================================================
        // GROUP ITEM + OPERATION BERDASARKAN SECTION
        // =========================================================
        $groupedData = [];
    ...
    <div class="action-bar no-print">
        <button onclick="window.print()" class="btn-print">
            <i class="ph-bold ph-printer"></i> Cetak BOM Produk (A4)
        </button>
        <br>
            ...
            <div class="content-wrapper">
                <div class="top-strip">
                    <div class="left">
                        ...
                        <span>Dokumen Master Resep Produksi</span>
                    </div>
                    <div class="right">
                ...
                <div class="header">
                    ...
                    <div class="doc-title">
                        <div class="doc-label">Manufacturing Recipe Document</div>
                        <h2>Master BOM</h2>
                        <img src="https://bwipjs-api.metafloor.com/?bcid=code128&text=<?= urlencode($bom['fg_sku'] ?? 'BOM') ?>&scale=2&height=8&includetext=false" alt="Barcode" class="barcode-img">
                        <p><?= esc($bom['fg_sku'] ?? 'BOM-PRODUK') ?></p>
                    </div>
                </div>
                    ...
                    <div class="meta-box alt">
                        ...
                        <table class="meta-table">
                            <tr>
                                ...
                                <td class="meta-val">: Master Bill of Materials</td>
                            </tr>
                            <tr>
                                <td class="meta-label">Nama Resep / SOP</td>
                                <td class="meta-val">: <?= esc($bom['recipe_name'] ?? '-') ?></td>
                            </tr>
                            <tr>
                                <td class="meta-label">Total Material</td>
                                <td class="meta-val">: <?= count($items) ?> Item Material</td>
                            </tr>
                        </table>
                    </div>

                    <div class="meta-box">
                        <h4><i class="ph-bold ph-package"></i> Produk Target</h4>
                        <table class="meta-table">
                            <tr>
                                <td class="meta-label">SKU Target</td>
                                <td class="meta-val" style="font-family: monospace;">: <?= esc($bom['fg_sku'] ?? '-') ?></td>
                            </tr>
                            <tr>
                                <td class="meta-label">Nama Produk</td>
                                <td class="meta-val bold">: <?= esc($bom['item_name'] ?? 'Produk Umum') ?></td>
                            </tr>
                            <tr>
                                <td class="meta-label">Kategori</td>
                                <td class="meta-val">: Produk Jadi (Finish Good)</td>
                            </tr>
                        </table>
                    </div>
                </div>
                        ...
                        <div class="table-wrap">
                            <table class="item-table">
                                <thead>
                                    <tr>
                                        <th width="6%">No</th>
                                        <th width="20%">SKU Material</th>
                                        <th width="34%">Deskripsi Material</th>
                                        <th width="20%">Ukuran / Rasio</th>
                                        <th width="20%">Target Kebutuhan</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php if (empty($group['items'])): ?>
                                        <tr>
                                            <td colspan="5" class="text-center" style="font-style: italic; color: var(--muted);">Tidak ada kebutuhan fisik/material.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php $no = 1; foreach ($group['items'] as $item): ?>
                                            <?php
                                                $sizePerItem = floatval($item['size_per_item'] ?? 0);
                                                $sizeUom     = trim($item['size_uom'] ?? '');
                                                $qtyPerItem  = floatval($item['qty_per_item'] ?? 0);
                                                $qtyUom      = trim($item['qty_uom'] ?? '');
                                                $qtyRequired = floatval($item['qty_required'] ?? 0);
                                                $unitFinal   = trim($item['unit'] ?? $qtyUom ?? 'PCS');
                                                ...
                                                $ukuranPart = ($sizePerItem > 0)
                                                    ? rtrim(rtrim(number_format($sizePerItem, 2, '.', ''), '0'), '.') . ' ' . $sizeUom
                                                    : '';

                                                $jumlahPart = ($qtyPerItem > 0)
                                                    ? rtrim(rtrim(number_format($qtyPerItem, 2, '.', ''), '0'), '.') . ' ' . $qtyUom
                                                    : '';

                                                if ($ukuranPart && $jumlahPart) {
                                                    $specText = $ukuranPart . ' × ' . $jumlahPart;
                                                } elseif ($ukuranPart) {
                                                    $specText = $ukuranPart;
                                                } elseif ($jumlahPart) {
                                                    $specText = $jumlahPart;
                                                ...
                                                $targetText = rtrim(rtrim(number_format($qtyRequired, 2, '.', ''), '0'), '.') . ' ' . $unitFinal;
                                            ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="product-inline">
                                                        <span class="desc-text"><?= esc($materialName) ?></span>
                                                    </div>
                                                </td>
                            ...
                            <table class="item-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="70%">Instruksi Operasi Kerja</th>
                                ...
                                <tbody>
                                    <?php if (empty($group['operations'])): ?>
                                        <tr>
                                            <td colspan="3" class="text-center" style="font-style: italic; color: var(--muted);">Tahapan routing belum tersedia.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($group['operations'] as $op): ?>
                                            <tr>
                                                <td class="text-center no-cell"><?= esc($op['step_order'] ?? '-') ?></td>
                                                <td>
                                                    <div class="product-inline">
                                                ...
                                                <td class="text-center">
                                                    <?php if (($op['is_final_step'] ?? 0) == 1): ?>
                                                        <span class="final-badge">TAHAP FINAL (GUDANG)</span>
                                                    <?php else: ?>
                                                        <span class="qty-target" style="background:#f8fafc; border-color:#cbd5e1; color:#0f172a;">Proses Tahap</span>
                                                    <?php endif; ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                ...
                <div class="notes-section">
                    <strong>Catatan Penggunaan BOM</strong>
                    <ol>
                        <li>Dokumen ini merupakan acuan standar komposisi material untuk proses produksi barang jadi.</li>
                        <li>Perubahan material, kuantitas, atau tahapan kerja wajib melalui approval bagian PPIC / Engineering.</li>
                        <li>Gunakan dokumen ini sebagai referensi utama saat menerbitkan SPK, Batch Produksi, dan pengeluaran bahan.</li>
                        <li>Pastikan semua material yang tercantum sesuai dengan master item dan stok gudang yang berlaku di sistem.</li>
                    </ol>
                </div>
                ...
                <div class="signature-section">
                    <div class="sig-box">
                        ...
                        <div class="sig-name">Admin / Engineering</div>
                    </div>
                    <div class="sig-box">
                        <p>Diperiksa Oleh</p>
                        <div class="sig-space"></div>
                        <div class="sig-line"></div>
                        <div class="sig-name">PPIC / Supervisor</div>
                    </div>
                    <div class="sig-box">
                        <p>Disetujui Oleh</p>
                        <div class="sig-space"></div>
                        <div class="sig-line"></div>
                        <div class="sig-name">Manager Produksi</div>
                    </div>
                </div>

                <div class="footer-note">
                    <span>Form No: BOM-MFG-001 • Rev: 03</span>
