...
<head>
    ...
    <style>
        ...
        .filter-bar { 
            ...
            display: flex; gap: 10px; margin-bottom: 25px; align-items: center; flex-wrap: wrap; 
        }
        .date-input { border: 1px solid #e5e7eb; background: var(--bg-light); font-weight: 600; padding: 10px 12px; border-radius: 8px; font-size: 13px; height: 42px; }
        .btn-apply { background: #1f2937; color: #fff; padding: 0 20px; border-radius: 8px; font-weight: 600; border: none; font-size: 13px; height: 42px; cursor: pointer; transition: 0.2s; white-space: nowrap; }
        ...
        /* TABLE WRAPPER */
        .report-card { background: #fff; border-radius: 16px; border: 1px solid #e5e7eb; overflow: hidden; margin-bottom: 30px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02); }
        
        .table-responsive { width: 100%; overflow-x: auto; -webkit-overflow-scrolling: touch; }
        
        .table-custom { width: 100%; border-collapse: collapse; min-width: 800px; /* Force scroll on mobile */ }
        .table-custom th { background: #f9fafb; text-align: left; padding: 15px; font-size: 11px; text-transform: uppercase; color: #6b7280; border-bottom: 1px solid #e5e7eb; vertical-align:middle; white-space: nowrap; }
        .table-custom td { padding: 15px; border-bottom: 1px solid #f3f4f6; font-size: 13px; color: #1f2937; vertical-align:middle; white-space: nowrap; }
        ...
        /* Badges */
        .badge-pill { padding: 4px 10px; border-radius: 6px; font-size: 10px; font-weight: 700; text-transform: uppercase; display: inline-block; min-width: 70px; text-align: center; white-space: nowrap; }
        ...
        @media (max-width: 768px) {
            .laporan-wrapper { padding: 15px; }
            .page-header-custom { flex-direction: column; align-items: flex-start; gap: 15px; }
            .page-header-custom button { width: 100%; }
            ...
            <div class="filter-bar no-print">
                <form method="GET" id="filterForm" style="display:flex; gap:10px; align-items:center; flex-wrap:wrap; width:100%;">
                    <div style="flex:1; min-width: 140px;">
                        <input type="date" name="tgl_awal" id="tgl_awal" value="<?php echo $tgl_awal; ?>" class="date-input" style="width:100%">
                    </div>
                    <span style="font-size:12px; font-weight:bold; color:#94a3b8;">s/d</span>
                    <div style="flex:1; min-width: 140px;">
                        <input type="date" name="tgl_akhir" id="tgl_akhir" value="<?php echo $tgl_akhir; ?>" class="date-input" style="width:100%">
                    </div>
                    <button type="submit" class="btn-apply" style="flex:0 0 auto;">Filter Data</button>
                </form>
            </div>
                    ...
                    <table class="table-custom">
                        ...
                        <tbody>
                            <?php if(empty($data_list)): ?>
                                <tr><td colspan="6" class="text-center p-5" style="padding:40px; color:#9ca3af;">Tidak ada data kasbon pada periode ini.</td></tr>
                            <?php else: ?>
                        ...
                        <tfoot style="background:#f9fafb; border-top:2px solid #e5e7eb;">
                            <tr>
                                <td colspan="4" class="text-right" style="font-weight:800; font-size:13px; color:#374151; padding-right:20px; padding:15px;">TOTAL SISA PIUTANG (APPROVED)</td>
                                <td class="text-right font-mono" style="font-size:14px; font-weight:800; color:#ef4444; padding:15px;">Rp <?php echo number_format($total_sisa); ?></td>
                                <td></td>
                            </tr>
                        </tfoot>
                    </table>
